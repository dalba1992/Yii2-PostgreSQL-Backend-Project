		...
		<div class="widget-title tradegecko_wrap">
			...
			<span class=pagination>
				...
				<?php if(isset($_GET['p']) && $_GET['p']> $prevpage ) {?> 
					<a class="btn" href="<?php echo Yii::$app->homeUrl.'/tradegecko/list?p='.$prevpage ?>">prev</a>
				<?php } ?>
				<?php  //if(sizeof($order['orders'])>0){?>
					<a class="btn" href="<?php echo Yii::$app->homeUrl.'/tradegecko/list?p='.$nextpage ?>">next</a>
				<?php //} ?>
			</span>
		</div>
					...
					<tbody>
						<?php
						 if(sizeof($order['orders'])>0){
							...
							#-----------------------------------------------------------------------		
					 		/*$Litevieworderlog = Litevieworderlog::find()
												 		->where(['tradegecko_order_id'=>$Tradegecko_orderinfo['id']])
							...
							<tr>
								...
								<td>
								<?php //Moved to the top: 
								$Litevieworderlog = Litevieworderlog::find()->where(['tradegecko_order_id'=>$Tradegecko_orderinfo['id']])->orderBy(['id'=>SORT_DESC])->one();
