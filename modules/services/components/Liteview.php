...
class Liteview extends Component {
    ...
    public function __construct($config = array()) {
        try{
            $this->appKey = Yii::$app->params['liteview']['apikey'];
            $this->username = Yii::$app->params['liteview']['username'];
        }catch(\Exception $ex){
            $this->appKey = '';
            $this->username = '';
        }
    ...
    public function submitOrder($params) {
        ...
        if(!$this->debug) {
            ...
            {
                $response['code'] = 0;
                $response['error'] = (string)$xml->error->error_description;
            }
            else
            {
                $response['code'] = 1;                
...
XML;
    ...
    public function cancelOrder($params) {
        ...
        if(!$this->debug) {
            ...
            {
                $response['code'] = 1;
                $details = $xml->cancel_order->order_info->order_details;
                $response['details'] = [
                    'status' => (string)$details->order_status,
                    'client_order_number' => (string)$details->client_order_number,
                    'ifs_order_number' => (string)$details->ifs_order_number
...
XML;
    ...
    public function orderStatus($order_number) {
        ...
        if(!$this->debug) {
            $params[] = $order_number;
            $responseXML = $this->sendRequest('order', 'ship_status', 'GET', $params);
...
XML;
    ...
    public function orderRetunsList($start_date, $end_date, $skip = 0) {
        ...
        if(!$this->debug) {
            ...
            $responseXML = $this->sendRequest('order', 'returns/list', 'GET', $params);
...
XML;
    ...
    public function getInventoryList($skip = 0) {
        ...
        if(!$this->debug) {
            ...
            $params[] = $skip;
            $responseXML = $this->sendRequest('inventory', 'list', 'GET', $params);
...
XML;
    ...
    protected function sendRequest($call, $func, $method = "GET", $params = []) {
        ...
        curl_setopt($curl, CURLOPT_HTTPHEADER, array ('Content-type: text/xml', 'appkey: ' . $this->appKey));
